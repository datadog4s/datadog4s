...
trait TimerFactory[F[_]] {
  ...
  /**
   * Create histogram-backed timer. The implications are that this the histograms are computed in datadog agent and not on DDog servers.
   * This means that the data in your dashboards etc might be less precise as they will be computed by aggregating histograms. For more info see
   ...
   * which means that you will have statistically correct data.
   *
