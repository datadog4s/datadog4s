...
@nowarn final case class MetricsOpsBuilder[F[_]: Sync] private (
  ...
  /** Force MetricOps to use [[api.TimerFactory.distribution TimerFactory.distribution]] for timing http4s requests. For
    * the implications please see [[api.TimerFactory.distribution TimerFactory.distribution]] scaladoc.
    */
  def useDistributionBasedTimers(): MetricsOpsBuilder[F] =
    copy(distributionBasedTimers = true)

  /** Force MetricOps to use [[api.TimerFactory.histogram TimerFactory.histogram]] for timing http4s requests. For the
    * implications please see [[api.TimerFactory.histogram TimerFactory.histogram]] scaladoc.
    */
  def useHistogramBasedTimers(): MetricsOpsBuilder[F] =
    copy(distributionBasedTimers = false)

  /** Force MetricOps to use [[api.DistributionFactory DistributionFactory]] for counting http4s requests. This is
  ...
  /** Force MetricOps to use [[api.metric.Count Count]] for counting http4s requests. This is sufficient for requests on
