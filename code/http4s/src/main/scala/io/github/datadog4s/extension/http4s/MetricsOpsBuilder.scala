...
@nowarn final case class MetricsOpsBuilder[F[_]: Sync] private (
  ...
  /** Force MetricOps to use [[io.github.datadog4s.api.TimerFactory.distribution TimerFactory.distribution]] for timing http4s requests. For
    * the implications please see [[io.github.datadog4s.api.TimerFactory.distribution TimerFactory.distribution]] scaladoc.
    */
  def useDistributionBasedTimers(): MetricsOpsBuilder[F] =
    copy(distributionBasedTimers = true)

  /** Force MetricOps to use [[io.github.datadog4s.api.TimerFactory.histogram TimerFactory.histogram]] for timing http4s requests. For the
    * implications please see [[io.github.datadog4s.api.TimerFactory.histogram TimerFactory.histogram]] scaladoc.
    */
  def useHistogramBasedTimers(): MetricsOpsBuilder[F] =
    copy(distributionBasedTimers = false)

  /** Force MetricOps to use [[io.github.datadog4s.api.DistributionFactory DistributionFactory]] for counting http4s requests. This is
    * useful in serverless models like AWS Lambda to aggregate all counter values. For more info see
  ...
  /** Force MetricOps to use [[io.github.datadog4s.api.metric.Count Count]] for counting http4s requests. This is sufficient for requests on
    * servers where the host is automatically added as a tag. For more info see
    * [[https://docs.datadoghq.com/serverless/custom_metrics/#understanding-distribution-metrics Datadog documentation]].
    */
